...
/*   Updated: 2025/08/19 18:32:02 by mhotting         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Span.hpp"

Span::Span(unsigned int maxSize): _maxSize(maxSize), _content(std::vector<int>()) {}

Span::Span(const Span& otherSpan): _maxSize(otherSpan._maxSize), _content(otherSpan._content) {}
...
long Span::shortestSpan(void) const {
    ...
    shortest = static_cast<long>(tempContent.at(1)) - static_cast<long>(tempContent.at(0));
    for (std::vector<int>::const_iterator it = tempContent.begin() + 1; it + 1 != tempContent.end(); it++) {
...
long Span::longestSpan(void) const {
    ...
    return static_cast<long>(*std::max_element(this->_content.begin(), this->_content.end())) - static_cast<long>(*std::min_element(this->_content.begin(), this->_content.end()));
}
